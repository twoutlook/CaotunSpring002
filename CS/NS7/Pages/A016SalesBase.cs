...
namespace NS7
{
    //   public class A015SalesBase : AppComponentBaseV7
    public class A016SalesBase : AppComponentBaseV7

    {
        // 1. 注入 Adapter, 每頁要有各自的, 不能混用
        [Inject]
        public A015Adapter Adapter { get; set; }
        ...
        protected override async Task ReloadAsync()
        {
            ...
            // (1) 總筆數
            PageHelper.TotalItemCount = await db.Sales.Where(qa.GetWhere()).CountAsync();

            // (2) 分頁的內容
            Items = db.Sales.Where(qa.GetWhere()).OrderBy(qa.GetOrderBy())
